...
export function ADSR({ envelope }: ADSRProps) {
    ...
    // The sketch is created once (memoized)
    // Live envelope values are read from the ref inside draw()
    const sketch = useCallback<Sketch>(
        (p) => {
            ...
            p.draw = () => {
                // Skip all work while scrolled off screen.
                if (!visibleRef.current) return;

                // Clear and redraw the static chrome each frame.
                drawStatic();

                // Live envelope values (updated by the sliders in Controls).
                const attack = envelope.current.attack as number;
                const decay = envelope.current.decay as number;
                const sustain = envelope.current.sustain as number;
                const release = envelope.current.release as number;
